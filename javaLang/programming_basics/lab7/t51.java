...
public class t51
{
    ...
    public static void task(int [][] matrix)
    {
        ...
        int mxMetrics[] = find(matrix, mx);
        int mxRow = mxMetrics[0];
        int mnMetrics[] = find(matrix, mn);
        int mnRow = mnMetrics[0];
        
        matrix = swapRows(matrix, mxRow, mnRow);
    ...
    public static int[][] swapRows(int matrix[][], int rowIdx1, int rowIdx2)
    {
        int columns = matrix[0].length;

        int tempRow[] = new int[columns];
        for (int i = 0; i < columns; i++) {
            tempRow[i] = matrix[rowIdx1][i];
        }
        
        for (int i = 0; i < columns; i++) {
            matrix[rowIdx1][i] = matrix[rowIdx2][i];
            matrix[rowIdx2][i] = tempRow[i];
        }
