class Task2:
    ...
    def do_task(self):
        ...
        while self.moves:
            if cnt % 2 == 0:
                max_move = max(self.moves)
                time += self.calculate_move(current_pos, max_move)
                current_pos = max_move
                del self.moves[self.moves.index(max_move)]
            else:
                min_move = min(self.moves)
                time += self.calculate_move(current_pos, min_move)
                current_pos = min_move
                del self.moves[self.moves.index(min_move)]

            cnt += 1
        time += self.calculate_move(current_pos, self.houses_amount)
